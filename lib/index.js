...
(0, project_1.getPackages)().then((packages) => {
    ...
    packages.forEach((pkg) => {
        ...
        const mn_devs = Object.entries(pkg.devDependencies)
            ...
            .filter((dev) => dev.startsWith('@moodlenet/'))
            .sort();
        (0, assert_1.default)(String(mn_peers) === String(mn_devs), `peers and dev deps not congruent in ${pkg.name}
...
function shouldSkip(pkg_or_name) {
    const pkg = typeof pkg_or_name === 'string'
